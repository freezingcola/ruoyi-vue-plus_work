...
const data = reactive<PageData<InfoLogForm, InfoLogQuery>>({
  ...
  rules: {
    ...
    workContent: [{ required: true, message: '内容不能为空', trigger: 'blur' }],
      state: [{ required: false, message: '标题不能为空', trigger: 'blur' }],
      attachs: [{ required: false, message: '标题不能为空', trigger: 'blur' }],
  }
});
...
const submitForm = () => {
  infoLogFormRef.value?.validate(async (valid: boolean) => {
      if ( form.value.workContent==undefined||form.value.workContent=="<p><br></p>"){
          ElMessage({
              message: '请输入内容',
              type: 'warning',
          })
